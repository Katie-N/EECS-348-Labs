...
int main() {
    int total;
    printf("Enter 0 or 1 to stop the program.\n");
    printf("Enter a score for an NFL game to know all the possible ways the score could be achieved: ");

    // If total scanf reads in an invalid character then we want to quit the program
    // This is a simple error check on the user input but it is not exhaustive
    if (scanf("%d", &total) != 1) {
        return 1;
    }
    while(total >= 2) {
        printf("To make %d points you could have:\nTD + 2\tTD + 1\tTD\tFG\tS\n", total);
        ...
        if (combos != 1) {
            // Use plural references
            printf("There are %d possible ways to score %d points\n", combos, total);
        } else {
            // Use singular references
            printf("There is %d way to score %d points\n", combos, total);
        }        

        printf("\nEnter a score: ");
        // Again we use a simple error check to make sure an integer was entered and end the program if it was not.
        if (scanf("%d", &total) != 1) {
            return 1;
        }
    }
    printf("You must enter at least 2 points! Ending program.\n");
